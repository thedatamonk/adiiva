...
class WebsocketClientApp {
  ...
  public async connect(): Promise<void> {
    try {
      ...
      // Step 1: Get worker URL from gateway
      this.log('Requesting worker from gateway...');
      ...
      if (!response.ok) {
        ...
        throw new Error(error.detail || `Gateway returned ${response.status}`);
      }

      const { worker_url, session_id } = await response.json();
      this.log(`Assigned worker: ${worker_url} (session: ${session_id})`);

      // Step 2: Connect directly to worker via WebSocket
